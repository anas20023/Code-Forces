...
void solve(){
    ...
    if(abs((ll)od.size()-(ll)ev.size())<=1){
        cout<<0<<endl;
        return;
    }
    ll l=min((ll)od.size(),(ll)ev.size());
    for (int i = 0; i < l+1 && i<od.size(); i++)
    {
        od[i]=0;
    }
    for (int i = 0; i < l+1 && i<ev.size(); i++)
    {
        ev[i]=0;
    }
    
    ll ans=accumulate(all(od),0);
  //  dbg(ans);
   ans+=accumulate(all(ev),0);
    cout<<ans<<endl;
